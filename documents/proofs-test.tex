...
\AxiomC{$P(A,x)$\e $Q(A),R(B)$}
\AxiomC{$Q(y)$ \e$Q(x)$}
\BinaryInfC{$P(x,x),Q(y)$\e$R(B)$}
\end{prooftree}

and now we cannot resolve away the $P(x,x)$.
...
There is a concern regarding unit formulas that have multiple literals; consider the unit being $P(a,b)$ being resolved in two places against $P(A, b)$ and $P(a, B)$, where the order of resolution in the proof matters (namely, other things are set to $a$ or $b$, respectively. I don't think this is as large an issue, because it seems that if this can be done with the non-lowered unit, then the proof can compensate in the lowered case as well.
