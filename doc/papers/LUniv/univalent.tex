...
operation, especially \emph{chain resolution} as defined in \cite{bwp10}, have been used by
the compression algorithms as in \cite{Amjad07} and \cite{Sinz}.  Cotton \cite{CottonSplit}
...
is the root of a subproof to be deleted. In that case, $\dn{\psi}{D}$ is defined as belonging to
$D$ in order to allow the deletion to be propagated down the proof. For instance, let's consider the
proof
...
For $n = 0$, $U = \varnothing$ and the properties are obvious. Suppose a subproof
...
proof exponentially bigger than the original. Some carefull \emph{partial regularization} algorithms
...
difficulty is that {\RPI} could create some new units which would be visible only after the
...
ratios on the second column corresponds to the formula (\ref{eq:compression}) in which $\psi$ ranges
...
the number of nodes. The compression speed on the fourth column
corresponds to the formula (\ref{eq:speed}) in which $d_{\psi}$ is the duration in milliseconds of
...
ratios differences in subfigure (c) are computed using the formula (\ref{eq:histogram}) with \va{LU}
...
The durations differences in subfigure (d) are computed using the same formula~(\ref{eq:histogram}) but
...
computation. And even if the compression ratio's gain is low on average, the subfigure (a) shows that
