...
A special case about deletion has to be noticed. Intuitively, deleting too many subproofs could
result in an inconsistant proof. It happens when on all path from the proof's root to each axiom,
the root of a subproof to be deleted lies. In that case, $\dn{\psi}{D}$ is defined as belonging to
$D$. This definition permits to propagate deletions down the proof. For instance, let's consider the
proof
$$ \psi = \dn{\varphi_1 \odot_\ell \varphi_2 \odot_{\ell'} \varphi_3}{\varphi_1, \varphi_2}. $$
As $\dn{\varphi_1 \odot_\ell \varphi_2}{\varphi_1, \varphi_2} \in \{\varphi_1, \varphi_2\}$,
$\psi = \dn{\varphi_3}{\varphi_1, \varphi_2}$ holds, as expected.
...
When $n = 0$ then $U = \varnothing$ and the properties are obvious.  Suppose a subproof
...
To address this issue, {\LowerUnivalents} traverses the proof in a top-down manner and
...
least one axiom, the average number of active literals is strictly less than two.  Therefore, if
{\LowerUnivalents} is implemented as an improved recursive \FuncSty{delete}, its time complexity
remains linear as long as membership of literals to the set $\Delta$ can be computed in constant
time.
...
exists though which achieve good compression ratio.

\texttt{RecyclePivotsWithIntersection} ({\RPI}), presented in \cite{LURPI}, is such a partial
regularization algorithm, which inhances the \texttt{RecyclePivots} algorithm (\cite{RP08}).  For
...
In a proof $\psi$, for all edge $\pedge{\varphi}{\ell}{\varphi'}$, $\ell$ is either a safe literal
...
effectively lower every univalent subproofs.
...
SMT-Lib\footnote{\url{http://www.smtlib.org/}}.  The details on the numbers of proofs by SMT logic
...
\begin{description}
  ...
  \item[Split:] teh Cotton's \texttt{Split} algorithm (\cite{CottonSplit});
...
ratios on the second column corresponds to the formula (\ref{eq:compression}) in which $\psi$ range
over all the proofs in the benchmark and $\psi'$ is the corresponding compressed proof.
\begin{equation} \label{eq:compression}
  1 - \frac{ \sum {|\Vertices{\psi}|} }{ \sum {|\Vertices{\psi'}|} }
\end{equation}
The unsat core compression ratios are computed the same way, using the number of axioms intead of
the number of nodes. The compression speed on the forth column
correspond to the formula (\ref{eq:speed}) in which $d_{\psi}$ is the duration in miliseconds of
...
ratio difference in subfigure (c) is computed using the formula (\ref{eq:histogram}) with \va{LU}
...
\end{equation}
The number of proof for which $\va{LU} = \va{LUniv}$ are not displayed in the histogram.
The duration difference in subfigure (d) is computed using the same formula (\ref{eq:histogram}) but
whith \va{LU} and \va{LUniv} being the duration of compressing a proof by {\LowerUnits} and
...
The algorithm we have presented have been shown to compress more than {\LowerUnits} because it
...
Moreover, the proposed algorithm can be embeded in the deletion traversal of another algorithm.  As
...
\begin{algorithm}[bt]
  ...
  \For{every subproof $\varphi$, in a top-down traversal  }{
    ...
    \tcp{Test if $\varphi$ is univalent.}
    \Activ $\leftarrow \varnothing$ \;
