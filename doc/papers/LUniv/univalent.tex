...
operation, in particular \emph{chain resolution} as defined in \cite{bwp10}, have been exploited by
the compression algorithms presented in \cite{Amjad07} and \cite{Sinz}.  Cotton \cite{CottonSplit}
...
stronger clauses and with fewer resolution steps. In \cite{RP} two linear time compression
...
In the next section, the propositional resolution calculus is introduced along with the notations,
...
$\dn{\psi}{\varphi_1 \ldots \varphi_n}$ being the result of \Rec{$\psi$,$\{\varphi_1, \ldots , \varphi_n\}$}. Both the resolution and deletion operations are considered to be left associative.
...
bottom of the resulting proof. Formaly, a subproof $\varphi$ in a proof $\psi$ can be lowered if there exists
...
{\LowerUnits} doesn't lower every lowerable subproof. In particular, it doesn't take into
...
U$. Suppose a subproof $\varphi_{n+1}$ of $\psi$ is univalent w.r.t. $\Delta_n$, with univalent
literal $\ell_{n+1}$.  Because $\ell_{n+1} \notin \Delta_n$, there exists a subproof of
$\dn{\psi}{U}$ with conclusion containing $\dual{\ell_{n+1}}$, and therefore
...
\dn{\dn{\psi}{U}}{\varphi_{n+1}} $ is included in $\Gamma \cup \{\dual{\ell_{n+1}}\}$. The conclusion
...
To address this issue, {\LowerUnivalents} traverses the proof in a top-down manner and simultaneously deletes
already collected univalent subproofs, as sketched in Algo. \ref{algo:LUniv}. Although the call to \FuncSty{delete} inside the first loop (line \ref{line:LUniv:step1begin} to \ref{line:LUniv:step1end}) suggests quadratic time complexity, this loop (line \ref{line:LUniv:step1begin} to \ref{line:LUniv:step1end}) can be (and has been)
actually implemented as a recursive function extending a recursive implementation of \FuncSty{delete}. With
such an implementation, {\LowerUnivalents} has a time complexity linear w.r.t. the size of
...
subproof may not be reintroduced. But this is not a problem, because it can be safely deleted.
...
sort the nodes of the graph first. But in case of sequential composition this costy operation has to
...
difficulty is that {\RPI} could create some new units which would be only visible after the
effective deletion.  A solution could be to test for units during deletion. But if units are
...
this traversal and would cause one more topological sorting to be performed because the deletion
deeply transforms the structure of the DAG.

Besides having an improved compression ratio, another advantage of {\LowerUnivalents} over {\LowerUnits} is that
{\LowerUnivalents} can be safely implemented as an enhanced \FuncSty{delete} operation.
...
After the first step of {\RPI}, as all edges labeled by a safe literal have been marked for deletion,
the remaining active literals are all valent. That means that {\LowerUnivalents} will effectively lower every univalent subproofs.
...
recursive \FuncSty{delete} improvement. But as Scala's set membership is $\mathcal{O}(\log{n})$,
{\LowerUnivalents}'s time complexity is $\mathcal{O}(n \log{n})$.

The algorithms have been experimented on 5\,059 proofs produced by the SMT-solver {\veriT}\footnote{\url{http://www.verit-solver.org/}} on unsatisfiable benchmarks from the SMT-Lib\footnote{\url{http://www.smtlib.org/}}.
The details on the numbers of proofs by SMT logic are shown on Table \ref{tab:benchmarks}.  The proofs were translated into pure resolution proofs by considering every non-resolution inference as an
axiom.
...
The experiments were executed on the Vienna Scientific
Cluster\footnote{\url{http://vsc.ac.at/}} VSC\nobreakdash-2. Each algorithm was executed in a single core. But the amount of memory available at each cluster node (16 GB) has been
usefull to compress the biggest proofs (with more than $10^6$ nodes).

\BWP{I think it is better to write a mathematical formula than to describe this formula in natural language}
...
the same way. The compression speed correspond to the number of the original proof's nodes analyzed \BWP{What do you mean by ``analyze''? Traversed nodes? Removed nodes? Could you write the formula for compression speed?}
in one millisecond.
...
compression difference and the duration difference are computed using the following formula, where $v$ is TODO...
...
\BWP{The introduction says: ``In this paper, a new algorithm generalizing {\LowerUnits} is described. The goals of this algorithm are to compress more than {\LowerUnits} and to allow fast non-sequential combination with {\RPI}''. In the conclusion, we must indicate that these goals have been achieved.} 

\BWP{Future work: are there ideas to lower even more nodes? }



\section*{Acknowledgments}

The authors would like to thank Pascal Fontaine for providing {\veriT}'s
proofs for the experiments, for co-organizing our joint workshops on proof compression\footnote{\url{http://www.logic.at/people/bruno/MediaWiki/index.php/Amadeus_Vienna-Nancy_Joint_Project_on_Proof_Compression}}, and for several interesting and useful discussions on this topic.
