...
\begin{equation}
  \psi = \varphi \odot_{\ell_n} \eta_n \cdots \odot_{\ell_1} \eta_1
\end{equation}
...
\begin{proposition}
If $\langle L,\Delta \rangle$ is a lowered part of $\psi$ below $\varphi$ and $\eta$ a subproof
in $\psi$ which is univalent w.r.t. $\Delta$, then $\eta$ is lowerable below $\varphi$.
\end{proposition}
