...
\varphi_n}$ denotes \Rec{$\psi$,$\{\varphi_1, \ldots , \varphi_n\}$}. It can easily be proved by
induction that $\dn{\psi}{A \cup B} = \dn{\left(\dn{\psi}{A}\right)}{B}$.
...
\begin{proposition}
For all proof $\psi$ and a set $D$ of $\psi$'s subproofs, let $\Gamma$ be the conclusion of $\psi$,
$\Gamma'$ the conclusion of $\dn{\psi}{D}$ and $\Delta$
the set of valent literals of $D$'s subproofs,
$$
\Delta = \{ \ell \ | \ \exists \varphi \in D, \ \ell \text{ is a valent literal of } \varphi \text{ in } \psi \}
$$
then
$$
\Gamma' \setminus \Gamma \subseteq \Delta
$$.
\end{proposition}
