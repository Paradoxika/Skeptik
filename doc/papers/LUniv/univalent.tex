...
$\varphi$ has one valent literal $\ell \notin \Delta$ in $\psi$ and $\Conclusion{\varphi} \subseteq
\Delta \cup \left\{ \ell \right\}$. $\ell$ is call the univalent literal of $\varphi$ in $\psi$
w.r.t.  $\Delta$.
\end{definition}
...
literal $\ell_{n+1}$.  There exists a subproof of $\psi$ which conclusion contains
$\dual{\ell_{n+1}}$, therefore $\dn{\psi}{U \cup \{\varphi_{n+1}\}} \notin U \cup
...
\{\dual{\ell{n+1}}\}$. The conclusion of $\psi' \odot_{\ell_{n+1}} \varphi_{n+1}$ is included in
...
If $\ell$ is not a valent literal of $\varphi$ but $\dual{\ell} \notin \Delta$, $\varphi$ will be
collected by {\LowerUnivalents} but not reintroduced.
If $\dual{\ell} \in \Delta$, $\varphi$ will be deleted by {\LowerUnivalents} because all edges
incoming to $\varphi$'s root are labeled with $\ell$. \qed
\end{proof}
