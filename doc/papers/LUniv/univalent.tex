...
\BWP{ ToDo: unsat core, interpolants... }
...
Algebraic properties of the resolution operation, in particular \emph{chain resolution} as defined
in \cite{bwp10}, have been exploited by the
compression algorithms presented in \cite{Amjad07} and \cite{Sinz}.
The \texttt{Split} (\cite{CottonSplit}) algorithm repeatedly constructs from the whole proof a subproof of $\ell$ and a
subproof of $\dual{\ell}$ for a randomly choosen literal $\ell$.
The \texttt{ReduceAndReconstruct} algorithm \cite{RedRec} repeatedly
performs atomic transformations on each resolution step.
This two repeating algorithm slowly converge to achieve good compression ratio.
In \cite{RP08} two linear time compression
...
above and the deletion of a set of subproofs from a proof. This last operation is noted
$\dn{\psi}{\varphi_1 \ldots \varphi_n}$ where $\psi$ is the whole proof and $\varphi_i$ are the
deleted subproofs. Algorithm \ref{algo:del} describes precisely the deletion operation,
$\dn{\psi}{\varphi_1 \ldots \varphi_n}$ being the result of \Rec{$\psi$,$\{\varphi_1, \ldots , \varphi_n\}$}.

Both the resolution and deletion operations are considered to be left associative.
...
of units to a second deletion traversal would suppress most of the benefits of combining.
...
Cluster\footnote{\url{http://vsc.ac.at/}} VSC\nobreakdash-2. None of the algorithms took advantage of the number
