...
%    A DAG with root is a tuple $\gamma = \langle V, E, L \rangle$ s.t. : \begin{itemize}
%      \item $V$ is a set of node,
%      \item $E \subset V \times X \times V$ a set of labeled directed edges
%      \item $L: V \longrightarrow Y$ a node labeling function,
%      \item $\rho(\gamma) \in V$ is a root.
%    \end{itemize}
%  \end{definition}

  \begin{definition}[Proof]
    A proof is a DAG $\varphi = \langle V, E, L \rangle$ with root $\rho(\varphi)$, which is either : \begin{itemize}
    \item an axiom $\langle \{v\}, \varnothing, v \longmapsto \Gamma \rangle$ for any clause
      $\Gamma$ and any new $v$;
    \item the resolution $\varphi_L \odot_\ell \varphi_R$ of proofs $\varphi_L = \langle V_L, E_L,
      L_L \rangle$ with $\dual{\ell} \in L_L(\rho(\varphi_L))$ and $\varphi_R = \langle V_R, E_R,
      L_R \rangle$ with $\ell \in L_R(\rho(\varphi_R))$ such that
      \begin{align*}
        V &= V_L \cup V_R \cup \{v\} \\
        E &= E_L \cup E_R \cup
             \{ \edge{v}{\dual{\ell}}{\rho(\varphi_L)}, \edge{v}{\ell}{\rho(\varphi_R)} \} \\
        L &= L_L \cup L_R \cup \{v \longmapsto \Gamma\}
      \end{align*} for any new $v$.
    \end{itemize}
  \end{definition}
\end{frame}
...
\begin{frame}
  \begin{definition}[Proof]
    ...
    \longrightarrow 2^\mathcal{L}$ a function labeling nodes with clauses, is a proof of $\Gamma$ iff either : \begin{itemize}
