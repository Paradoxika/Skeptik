...
In a proof $\psi[\eta]$ of $\Sigma$, a literal $p$ is safe for $\eta$ proving $\Gamma$ if
for all proofs $\eta'$ of $\Gamma \cup \{p\}$, $\psi[\eta']$ still proves $\Sigma$.
\end{definition}
...
\begin{enumerate}%%[nosep]
  ...
  \item $\mathcal{E} \subset \mathcal{V} \times \mathcal{V}$ is the \emph{premise} relation.
    ...
    \BW{$\xrightarrow{*}$ is defined as the transitive closure here. But in proposition 1, you seem to need it to be the transitive reflexive closure. Usually, $\xrightarrow{+}$ is used for the transitive closure and $\xrightarrow{*}$ for the transitive reflexive closure.}
...
\begin{itemize}%%[nosep]
  \item Let $\mathcal{V}' = \{\eta|\rho \xrightarrow{*} \eta\}$.
\BW{When transforming a pseudo-proof into a proof, you also want to get rid of nodes that have only one parent. The definition of $\mathcal{V}'$ here does not do that.}
   If $\mathcal{V}' \neq \mathcal{V}$
...
\end{algorithm}
\BW{A pseudo-code is not an implementation. I suggest to have simply the name of the algorithm in the caption.}
