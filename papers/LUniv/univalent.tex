...
\newcommand{\litr}[1]{\ensuremath{\mathcal{#1}}}
...
In a proof $\psi[\eta]$ of $\Sigma$, a literal $p$ is safe for the node $\eta$ proving $\Gamma$ if for all proof $\eta'$ of $\Gamma \cup \{p\}$, $\psi[\eta']$ still proves $\Sigma$.
\end{definition}
...
\begin{algorithm}[hbt]
  \For{every resolution node $\eta$ in a top-down traversal}{
    ...
    \uIf{the left premise of $\eta$ is deleted or the right premise's clause doesn't contain $p$}{
      Replace $\eta$ by its right premise \; }
    \uElseIf{the right premise is deleted or the left premise's conclusion doesn't contain $\dual{p}$}{
