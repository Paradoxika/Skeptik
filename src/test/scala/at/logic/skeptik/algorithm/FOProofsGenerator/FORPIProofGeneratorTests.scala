...
//  //as such proofs do not contain resolution nodes anyway
  ...
  def compressionTests(proofN: Int, height: Int, timeout: Int): Unit = {
    ...
    val outfile = "random-results-" + startTime + ".txt"
...
//      var luRPIproof: Proof[Node] = null
      ...
      stats.print(",")
      printStats(stats, proof, luRPIProof, LURPIfail, rpiTime + lurpiTime, lurpiFailString)
      stats.println("," + totalTime + "," + RPIfailAfterLU + "," + LUfailAfterRPI + "," + proofFile)
