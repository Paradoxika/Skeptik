...
object FOLowerUnits
  ...
  def checkContraction(listOfUnits: List[E], vars: MSet[Var], node: SequentProofNode): Boolean = {
    ...
    val con = try {
      contractIfHelpful(Axiom(newSeq),true)(vars)
    } catch {
      case e: Exception => {
        null
      }
              ...
              if (e.getMessage() != null) {
                if (e.getMessage.contains("Resolution (MRR): the resolvent is ambiguous.")) {
                  ...
                  try {
                    ...
                    case f: Exception => {
                      //Note that this is necessary because we might miss this case 
                      //(findOriginalResolvent uses different left/right arguments)

                      try {
                        UnifyingResolutionMRR(fixedLeft, fixedRight)(vars)

                        //And if that was ambiguous, we try using subs...just as before //TODO: refactor this.
                      } catch {
                        case h: Exception => {
                          if (findRenaming(fixedLeft.conclusion, left.conclusion)(vars) != null &&
                            findRenaming(fixedRight.conclusion, right.conclusion)(vars) != null) {
  ...
  def resolveUsingMultipleCarriedSubs(node: SequentProofNode, fixedRight: SequentProofNode, fixedLeft: SequentProofNode, right: SequentProofNode, left: SequentProofNode,
    ...
    val contractedFixedLeft = contractIfHelpful(newFixedLeft,true)(vars)
    val contractedFixedRight = contractIfHelpful(newFixedRight,true)(vars)

    val out = try {
      val contracted = contractIfHelpful(Axiom(newGoalD),true)(vars)
      try{
    ...
    val outContracted = contractIfHelpful(out,true)(vars)
    ...
    val contractedGoal = if (!contracted) {
      ...
      if (newGoalIfDesperate == null) {
        contractIfHelpful(Axiom(out.conclusion),true)(vars).conclusion
      } else {
        contractIfHelpful(Axiom(newGoalIfDesperate),true)(vars).conclusion
      }
    }
      ...
      for (rightCarry <- finalRightCarries) {
        try {
          ...
          val contractedNewLeft = if (attemptContraction) {
            contractIfHelpful(fixedLeft,true)(vars)
          } else {
            fixedLeft
          }
          val contractedNewRight = if (attemptContraction) {
            contractIfHelpful(fixedRight,true)(vars)
          } else {
            fixedRight
          }
  ...
  def findUnitInSeq(node: SequentProofNode, units: List[SequentProofNode], vars: MSet[Var]): SequentProofNode = {
    if (units.size < 1) {
      ...
      if (unitToTest.conclusion.suc.size > 0) {
        val unitFormulas = findUnifiableFormula(unitToTest.conclusion.suc.head, node.conclusion.ant)(vars)
        ...
        val unitFormulas = findUnifiableFormula(unitToTest.conclusion.ant.head, node.conclusion.suc)(vars)
        if (unitFormulas.size > 0) {
          unitToTest
        } else {
          findUnitInSeq(node, units.tail, vars)
  ...
  def smartContraction(left: SequentProofNode, right: SequentProofNode, units: List[SequentProofNode], vars: MSet[Var]): SequentProofNode = {
    val newRight = {
      contractIfHelpful(right,true)(vars)
    }
    ...
    val newLeftCon = contractIfHelpful(newLeftAx,true)(vars)
    ...
    val leftContractedSmart = contractIfHelpful(leftSubstituted,true)(vars) //TODO: should this applied to the right as well?
  ...
  def contractAndUnify(left: SequentProofNode, right: SequentProofNode, vars: MSet[Var], units: List[SequentProofNode]) = {
    if (isUnitClause(left.conclusion)) {
      if (isUnitClause(right.conclusion)) {
        ...
        val contracted = contractIfHelpful(right,true)(vars) 
        finishResolution(left, contracted, true)(vars)
      }
    } else {
      if (isUnitClause(right.conclusion)) {
        //only left is non-unit
        val contracted = contractIfHelpful(left,true)(vars)
        finishResolution(contracted, right, false)(vars)
      } else {
        //both are non-units
        val contractedL = contractIfHelpful(left,true)(vars)
        val contractedR = contractIfHelpful(right,true)(vars)
  ...
  def finishResolution(left: SequentProofNode, right: SequentProofNode, leftIsUnit: Boolean)(implicit unifiableVariables: MSet[Var]): SequentProofNode = {
    try {
      finishResolutionHelper(left, right, leftIsUnit)
    } catch {
      case e: Exception => {
        finishResolutionHelper(right, left, !leftIsUnit)
      }
    }
  }

  def finishResolutionHelper(left: SequentProofNode, right: SequentProofNode, leftIsUnit: Boolean)(implicit unifiableVariables: MSet[Var]): SequentProofNode = {
    try {
