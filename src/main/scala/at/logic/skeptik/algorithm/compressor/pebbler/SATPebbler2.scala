...
//    println(axioms)
//    
//    println(proof.nodes.map(n - proof.nodes.indexOf(_)))
//    
//    println(proof.nodes(0))
...
//      (1 to n).foreach(j => {
//        (1 to k).foreach(x => {
//          println((x,j,t) + " -> " + p(x,j,t) + " -> " + intToNode(p(x,j,t),n,k))
//        })
//      })
//    })
...
//    def notTwoAtTime2(values: Seq[Int]) = {
//      -p(values(0),values(1),values(2)) + " " + -p(values(3), values(4), values(5)) + " "
//    }
    ...
    def oneNodePerPebble = {
      (1 to k).map(x => {
        (1 to n).map(j => {
          (1 to n).map(t => {
            (1 to (j-1)).map(i => {
//              println(notP1orNotP2(x,j,t,x,i,t) + " j: "+ j + ", i: " + i + " p(x,j,t): " + p(x,j,t) + ", p(x,i,t)" + p(x,i,t))
//              clauses = clauses + 1
...
//              println("premise: " + i + " node: " + j)
            ...
            (1 to n).map(i => {
              (1 to (x-1)).map(y => {
//                println(p(x,j,t) + " " + -p(x,j,(t+1)) + " " + p(y,i,t) + " " + -p(y,i,(t+1)) + " 0\n")
//                clauses = clauses + 1
...
//    out.prepend("p cnf " + (k*n*n) + " " + clauses +"\n")
//    println("header written")
    
//    "p cnf " + (k*n*n) + " " + clauses +"\n" + 
//    "c one node per pebble\n" + one +
//    "c at least 1 init\n" + atL1 +
//    "c at most 1 init\n" + atM1 +
//    "c only axioms init\n" + axi +
//    "c correct pebbling\n" + pebb +
